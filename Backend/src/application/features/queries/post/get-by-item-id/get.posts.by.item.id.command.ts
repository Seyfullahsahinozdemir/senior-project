...
export function makeGetPostsByItemIdCommand({
  postService,
  userRepository,
  imageService,
  itemRepository,
  ...
  return async function getPostsByItemIdCommand(
    command: { pageIndex?: string; pageSize?: string; itemId: string },
    res: Response,
  ) {
    await validate(command);
    const posts = await postService.getPostsByItemId(
